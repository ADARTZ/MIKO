...
// friday's commands
let miniComs = [];
miniComs.push("hello mini");
miniComs.push("what are your commands");
miniComs.push("close this - to close opened popups");
miniComs.push(
  "change my information - information regarding your acoounts and you"
);
miniComs.push("whats the weather or temperature");
miniComs.push("show the full weather report");
miniComs.push("are you there - to check fridays presence");
miniComs.push("shut down - stop voice recognition");
miniComs.push("open google");
miniComs.push('search for "your keywords" - to search on google ');
miniComs.push("open whatsapp");
miniComs.push("open youtube");
miniComs.push('play "your keywords" - to search on youtube ');
miniComs.push("close this youtube tab - to close opened youtube tab");
miniComs.push("open firebase");
miniComs.push("open netlify");
miniComs.push("open twitter");
miniComs.push("open my twitter profile");
miniComs.push("open instagram");
miniComs.push("open my instagram profile");
miniComs.push("open github");
miniComs.push("open my github profile");
...
recognition.onresult = function (event) {
  ...
  // some casual commands
    ...
    if (transcript.includes("tell me about yourself")) {
      readOut(
        "sir, i am a jarvis, a voice assistant named as Mini made for browsers using javascript by adarsh prasad. I can do anything which can be done from a browser."
      );
    }
    ...
    if (transcript.includes("sleep now")) {
  ...
  // spotify
    ...
    if (transcript.includes("what's my bio")) {
      readOut(`Sir, based on your information you are ${JSON.parse(userData).bio}`);
    }
    if (transcript.includes("what's my location")) {
      readOut(`Sir, based on your information your location is ${JSON.parse(userData).location}`);
    }
  ...
  if(localStorage.getItem("lang") === "hi-IN"){
    if(transcript.includes("हेलो जार्विस")){
      readOutHindi("हेलो सर")
    }

    if(transcript.includes("इंग्लिश में बदलो")){
      readOutHindi("इंग्लिश में बदल रही हूँ")
      speech_lang = "en-US"
      localStorage.setItem("lang", "en-US")
      stopingR = true
      recognition.stop()
    ...
    if (transcript.includes("अपने बारे में कुछ बताइए")) {
      readOutHindi("मैं बहुत समयनिष्ठ हूँ और सही समय पर पूरे दिन के सभी कार्य को करना पसंद करती हूँ।");
    }
